...
function makeBufferSegment(s, encoding, nullTerminate = false) {
    return [0 /* SegmentType.Buffer */, makeBuffer(s, encoding, nullTerminate)];
}
function getSegmentSize(segment, value) {
    switch (segment) {
        case 0 /* SegmentType.Buffer */: {
        ...
        case 6 /* SegmentType.Int64BE */:
        case 2 /* SegmentType.Float8 */: {
            return 8;
        }
        case 3 /* SegmentType.Int8 */: {
            return 1;
        }
        case 4 /* SegmentType.Int16BE */: {
            return 2;
        }
        case 1 /* SegmentType.Float4 */:
        case 5 /* SegmentType.Int32BE */:
        case 7 /* SegmentType.UInt32BE */: {
            return 4;
        }
    }
    return -1;
...
function readRowData(buffer, row, columnSpecification, encoding, bigints, types, streams) {
    ...
    while (i < columns) {
        ...
        if (start < end) {
            ...
            let skip = false;
            if (streams && spec === types_1.DataType.Bytea) {
            ...
            if (!skip) {
                ...
                if (isReader) {
                    const reader = types === null || types === void 0 ? void 0 : types.get(dataType);
                    if (reader) {
...
class Writer {
    constructor(encoding) {
        ...
        this.outgoing = new buffer_1.ElasticBuffer();
    }
    bind(name, portal, format = types_1.DataFormat.Binary, values = [], types = []) {
        ...
        const segments = [
            ...
            [4 /* SegmentType.Int16BE */, length]
        ];
        const getFormat = (typeof format === 'number') ?
            () => format :
            (i) => format[i];
        for (let i = 0; i < length; i++) {
            segments.push([4 /* SegmentType.Int16BE */, getFormat(i)]);
        }
        segments.push([4 /* SegmentType.Int16BE */, length]);
        ...
        const addBinaryValue = (value, dataType) => {
            let size = -1;
            const setSize = reserve(5 /* SegmentType.Int32BE */);
            if (value === null) {
                setSize(-1);
                return 0;
            }
            switch (dataType) {
                case types_1.DataType.Bool: {
                    size = add(3 /* SegmentType.Int8 */, (value) ? 1 : 0);
                    break;
                }
                case types_1.DataType.Date: {
                    if (value === infinity) {
                        size = add(5 /* SegmentType.Int32BE */, 0x7fffffff);
                    }
                    else if (value === -infinity) {
                        size = add(5 /* SegmentType.Int32BE */, -0x80000000);
                    }
                    else if (value instanceof Date) {
                        size = add(5 /* SegmentType.Int32BE */, (value.getTime() - timeshift) /
                            (1000 * 86400));
                    }
                    break;
                }
                ...
                case types_1.DataType.Timestamptz: {
                    if (value === infinity) {
                        size = (0, utils_1.sum)(add(7 /* SegmentType.UInt32BE */, 0x7fffffff), add(7 /* SegmentType.UInt32BE */, 0xffffffff));
                    }
                    else if (value === -infinity) {
                        size = (0, utils_1.sum)(add(7 /* SegmentType.UInt32BE */, 0x80000000), add(7 /* SegmentType.UInt32BE */, 0x00000000));
                    }
                    else if (value instanceof Date) {
                        const n = (value.getTime() - timeshift) * 1000;
                        const f = Math.floor(n / 4294967296);
                        const r = n - f * 4294967296;
                        size = (0, utils_1.sum)(add(5 /* SegmentType.Int32BE */, f), add(7 /* SegmentType.UInt32BE */, r));
                    }
                    break;
                }
                case types_1.DataType.Bpchar:
                ...
                case types_1.DataType.Varchar: {
                    if (value instanceof Buffer) {
                        size = add(0 /* SegmentType.Buffer */, value);
                    }
                    else {
                        const s = String(value);
                        size = add(0 /* SegmentType.Buffer */, makeBuffer(s, this.encoding));
                    }
                    break;
                }
                case types_1.DataType.Float4: {
                    size = add(1 /* SegmentType.Float4 */, Number(value));
                    break;
                }
                case types_1.DataType.Float8: {
                    size = add(2 /* SegmentType.Float8 */, Number(value));
                    break;
                }
                case types_1.DataType.Int2: {
                    size = add(4 /* SegmentType.Int16BE */, Number(value));
                    break;
                }
                case types_1.DataType.Int4:
                case types_1.DataType.Oid: {
                    size = add(5 /* SegmentType.Int32BE */, Number(value));
                    break;
                }
                case types_1.DataType.Int8: {
                    size = add(6 /* SegmentType.Int64BE */, value instanceof Buffer ? value.readBigInt64BE(0) : (typeof value === "bigint" ? value : Number(value)));
                    break;
                }
                case types_1.DataType.Point: {
                    if ((0, types_1.isPoint)(value)) {
                        size = (0, utils_1.sum)(add(2 /* SegmentType.Float8 */, value.x), add(2 /* SegmentType.Float8 */, value.y));
                    }
                    break;
                }
                case types_1.DataType.Jsonb:
                    const body = JSON.stringify(value);
                    add(3 /* SegmentType.Int8 */, 0x01);
                    size = 1 + add(0 /* SegmentType.Buffer */, makeBuffer(body, this.encoding));
                    break;
                case types_1.DataType.Json: {
                    const body = JSON.stringify(value);
                    size = add(0 /* SegmentType.Buffer */, makeBuffer(body, this.encoding));
                    break;
                }
                case types_1.DataType.Uuid: {
                    try {
                        if (typeof value === 'string') {
                            ...
                            size = add(0 /* SegmentType.Buffer */, buffer);
                        }
                    }
                    catch (error) {
        ...
        const addBinaryArray = (value, dataType) => {
            const setDimCount = reserve(5 /* SegmentType.Int32BE */);
            add(5 /* SegmentType.Int32BE */, 1);
            add(5 /* SegmentType.Int32BE */, dataType);
            let bytes = 12;
            let dimCount = 0;
            const go = (level, value) => {
                ...
                if (length === 0)
                    return;
                if (level === dimCount) {
                    bytes += (0, utils_1.sum)(add(5 /* SegmentType.Int32BE */, length), add(5 /* SegmentType.Int32BE */, 1));
                    dimCount++;
                }
                for (let i = 0; i < length; i++) {
                    const v = value[i];
        ...
        for (let i = 0; i < length; i++) {
            ...
            if (format === types_1.DataFormat.Binary) {
                ...
                const size = (result instanceof Array) ?
                    (0, utils_1.sum)(...result.map((s) => add(0 /* SegmentType.Buffer */, makeBuffer(s, this.encoding)))) :
                    add(0 /* SegmentType.Buffer */, (result === null) ?
                        nullBuffer :
                        makeBuffer(result, this.encoding));
                setSize(size);
            }
        }
        add(4 /* SegmentType.Int16BE */, 1);
        add(4 /* SegmentType.Int16BE */, 1);
        this.enqueue(66 /* Command.Bind */, segments);
    }
    close(name, kind) {
        this.enqueue(67 /* Command.Close */, [
            makeBufferSegment(kind + name, this.encoding, true)
        ]);
    }
    describe(name, kind) {
        this.enqueue(68 /* Command.Describe */, [
            makeBufferSegment(kind + name, this.encoding, true)
        ]);
    }
    execute(portal, limit = 0) {
        this.enqueue(69 /* Command.Execute */, [
            makeBufferSegment(portal, this.encoding, true),
            [5 /* SegmentType.Int32BE */, limit],
        ]);
    }
    end() {
        this.enqueue(88 /* Command.End */, []);
    }
    flush() {
        this.enqueue(72 /* Command.Flush */, []);
    }
    parse(name, text, types = []) {
        ...
        for (let i = 0; i < length; i++) {
            segments.push([5 /* SegmentType.Int32BE */, types[i]]);
        }
        this.enqueue(80 /* Command.Parse */, segments);
    }
    password(text) {
        this.enqueue(112 /* Command.Password */, [makeBufferSegment(text, this.encoding, true)]);
    }
    saslInitialResponse(mechanism, clientNonce) {
        if (mechanism !== 'SCRAM-SHA-256')
            return false;
        const response = Buffer.from('n,,n=*,r=' + clientNonce);
        this.enqueue(112 /* SASL.SASLResponse */, [
            makeBufferSegment(mechanism, this.encoding, true),
            [5 /* SegmentType.Int32BE */, response.length],
            [0 /* SegmentType.Buffer */, response]
        ]);
        return true;
    }
    saslResponse(data, password, clientNonce) {
        const [response, signature] = (0, sasl_1.sign)(data, password, clientNonce);
        this.enqueue(112 /* SASL.SASLResponse */, [
    ...
    startup(settings) {
        ...
        data.push('');
        const segments = [
            [4 /* SegmentType.Int16BE */, 3],
            [4 /* SegmentType.Int16BE */, 0]
        ];
        for (const s of data) {
    ...
    startupSSL() {
        const segments = [
            [4 /* SegmentType.Int16BE */, 0x04D2],
            [4 /* SegmentType.Int16BE */, 0x162F]
        ];
        this.enqueue(null, segments);
    }
    sync() {
        this.enqueue(83 /* Command.Sync */, []);
    }
