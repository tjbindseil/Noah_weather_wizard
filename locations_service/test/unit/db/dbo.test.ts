...
describe("dbo tests", () => {
  ...
  //     {
  //       create: async () => {
  //         const client = new Client(get_app_config().pictureDbConnectionConfig);
  ...
  //       await client.query<Invite>('delete from invite where "id" in ($1)', [
  //         inviteIdsToDelete.at(i),
  //       ]);
  //     }
  //     for (let i = 0; i < pictureIdsToDelete.length; ++i) {
  //       await client.query<Picture>('delete from picture where "id" in ($1)', [
  ...
  //     const initialPictures = await getPictures(client, insertTestBucketName, "");
  ...
  //     pictureIdsToDelete.push(inserted.id);
  //
  //     const finalPictures = await getPictures(client, insertTestBucketName, "");
  ...
  //
  //     const finalDeletePictures = await getPictures(client, deleteBucketName, "");
